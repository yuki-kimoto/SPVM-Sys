...
{
  {
    my $exit_success_program = "$^X -Mblib $FindBin::Bin/sys_exec_with_program.pl";
    my $output = `$exit_success_program`;
    is($output, 'Hello abc');
    ok($? >> 8 == POSIX::EXIT_SUCCESS);
  }
...
if ($^O eq 'MSWin32') {
  ok(SPVM::TestCase::Sys::Process->spawnvp_nowait);
}
else {
  diag "[Test Skip]spawnvp_nowait is not supported in this system.";
}
