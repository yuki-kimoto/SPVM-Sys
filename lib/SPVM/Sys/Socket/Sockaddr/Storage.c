...
int32_t SPVM__Sys__Socket__Sockaddr__Storage__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  struct sockaddr_strage* socket_address = env->new_memory_stack(env, stack, sizeof(struct sockaddr_strage));
...
int32_t SPVM__Sys__Socket__Sockaddr__Storage__DESTROY(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  struct sockaddr_strage* socket_address = env->get_pointer(env, stack, obj_socket_address);
