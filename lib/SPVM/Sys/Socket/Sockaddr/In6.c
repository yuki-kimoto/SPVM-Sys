...
static const char* FILE_NAME = "Sys/Socket/Sockaddr/In6.c";

int32_t SPVM__Sys__Socket__Sockaddr__In6__new(SPVM_ENV* env, SPVM_VALUE* stack) {
  
  int32_t error_id = 0;
  
  struct sockaddr_in6* socket_address = env->new_memory_block(env, stack, sizeof(struct sockaddr_in6));

  SPVM_OBJ* obj_socket_address = env->new_pointer_object_by_name(env, stack, "Sys::Socket::Sockaddr::In6", socket_address, &error_id, __func__, FILE_NAME, __LINE__);
...
int32_t SPVM__Sys__Socket__Sockaddr__In6__size(SPVM_ENV* env, SPVM_VALUE* stack) {
