...
int32_t SPVM__Sys__Signal__Handler__eq(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!obj_handler1) {
    return env->die_v2(env, stack, "The handler $handler1 must be defined.", __func__, FILE_NAME, __LINE__);
  }
  
  void* obj_handler2 = stack[1].oval;
  
  if (!obj_handler2) {
    return env->die_v2(env, stack, "The handler $handler2 must be defined.", __func__, FILE_NAME, __LINE__);
  }
