...
int32_t SPVM__Sys__Poll__PollfdArray__fd(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(index >= 0)) {
    return env->die_v2(env, stack, "The index $index must be greater than or equal to 0.", __func__, FILE_NAME, __LINE__);
  }
  
  if (!(index < length)) {
    return env->die_v2(env, stack, "The index $index must be less than the value of the length field.", __func__, FILE_NAME, __LINE__);
  }
...
int32_t SPVM__Sys__Poll__PollfdArray__remove(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(index < length_field)) {
    return env->die_v2(env, stack, "The index $index must be less than the value of length field.", __func__, FILE_NAME, __LINE__);
  }
