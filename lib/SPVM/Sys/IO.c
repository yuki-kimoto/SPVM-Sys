...
int32_t SPVM__Sys__IO__FD_SET(SPVM_ENV* env, SPVM_VALUE* stack) {
...
int32_t SPVM__Sys__IO__FD_CLR(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(fd < FD_SETSIZE)) {
    return env->die(env, stack, "The $fd must be less than FD_CLRSIZE", FILE_NAME, __LINE__);
  }
...
int32_t SPVM__Sys__IO__FD_ISSET(SPVM_ENV* env, SPVM_VALUE* stack) {
  ...
  if (!(fd < FD_SETSIZE)) {
    return env->die(env, stack, "The $fd must be less than FD_ISSETSIZE", FILE_NAME, __LINE__);
  }
